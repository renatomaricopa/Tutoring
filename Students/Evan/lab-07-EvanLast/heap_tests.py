...
class TestHeap(unittest.TestCase):
    ...
    def test_03_heap_contents(self):
        ...
        self.assertEqual(test_heap.contents(), [3, 2, 1]) # this is for bottom up construction
        # self.assertEqual(test_heap.contents(), [3, 1, 2])  # this is for top down construction
    ...
    def test_12_check_duplicates(self):
        test_heap = MaxHeap(10)
        test_heap.build_heap([2, 9, 8, 3, 3, 8])
        self.assertEqual(test_heap.contents(), [9, 3, 8, 2, 3, 8])
    def test_13_peek(self):
        test_heap = MaxHeap(10)
        test_heap2 = MaxHeap(10)
        test_heap.build_heap([])
        test_heap2.build_heap([1])
        self.assertEqual(test_heap.peek(), False)
        self.assertEqual(test_heap2.peek(), True)
